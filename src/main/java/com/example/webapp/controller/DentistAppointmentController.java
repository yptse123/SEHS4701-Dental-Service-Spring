...
public class DentistAppointmentController {
    ...
    public String viewAppointment(@PathVariable("id") Long id, Model model) {
        ...
        String createdAtFormatted = appointment.getCreatedAt() != null
                ? appointment.getCreatedAt().format(dateFormatter)
                : null;
        String updatedAtFormatted = appointment.getUpdatedAt() != null
                ? appointment.getUpdatedAt().format(dateFormatter)
                : null;
        ...
        List<Appointment> recentAppointments = appointmentService.findByPatient(appointment.getPatient(), 5);
    ...
    public String viewPatientDetails(@PathVariable("id") Long id, Model model) {
        // Get current authenticated dentist
        Dentist dentist = getCurrentDentist();
        ...
        List<Appointment> appointments = appointmentService.findByPatientAndDentist(patient, dentist);

        // Add data to model
        model.addAttribute("patient", patient);
        model.addAttribute("appointments", appointments);

        return "dentist/patient-details";
    }
}